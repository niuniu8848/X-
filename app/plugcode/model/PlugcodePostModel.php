 ...
 * @property mixed id
 */
class PlugcodePostModel extends Model
{
    ...
    {
        return $this->belongsToMany('PlugcodeCategoryModel', 'plugcode_category_post', 'category_id', 'post_id');
    }
    /**
     * 关联标签表
     * @return \think\model\relation\BelongsToMany
    ...
    {
        return $this->belongsToMany('PlugcodeTagModel', 'plugcode_tag_post', 'tag_id', 'post_id');
    }
    ...
    {
        $portalTagModel = new PlugcodeTagModel();
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('plugcode_tag_post')->where('post_id', $articleId)->column('tag_id');
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('plugcode_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($data)) {
                Db::name('plugcode_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('plugcode_post')->where('id', $id)->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'plugcode_post';
                ...
                $transStatus = false;
                try {
                    Db::name('plugcode_post')->where('id', 'in', $ids)
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'plugcode/Page/index', ['id' => $data['id']], 2, 5000);
