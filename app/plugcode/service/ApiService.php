...
// +----------------------------------------------------------------------
namespace app\plugcode\service;
...
class ApiService
{
    /**
     * 功能:查询文章列表,支持分页;<br>
     * 注:此方法查询时关联两个表plugcode_category_post(category_post),plugcode_post(post);在指定排序(order),指定查询条件(where)最好指定一下表别名
     ...
     *                     此方法查询时关联两个表plugcode_category_post(category_post),plugcode_post(post);
     *                     所以最好指定一下表名,以防字段冲突
    ...
    public static function articles($param)
    {
        ...
        if (!empty($categoryIds)) {
            ...
            array_push($join, ['__PLUGCODE_CATEGORY_POST__ category_post', 'post.id = category_post.post_id']);
            ...
            $field = !empty($param['field']) ? $param['field'] : 'post.*,min(category_post.category_id) as category_id,user.user_login,user.more as user_more';
            array_push($join, ['__PLUGCODE_CATEGORY_POST__ category_post', 'post.id = category_post.post_id']);
        }
    ...
    /**
     * 功能:查询标签文章列表,支持分页;<br>
     * 注:此方法查询时关联两个表plugcode_tag_post(tag_post),plugcode_post(post);在指定排序(order),指定查询条件(where)最好指定一下表别名
     * @param array $param 查询参数<pre>
     *                     array(
     *                     'tag_id'=>'',
     *                     'where'=>'',
     ...
     *                     field:调用指定的字段@todo
     *                     如只调用posts表里的id和post_title字段可以是post.id,post.post_title; 默认全部,
     *                     此方法查询时关联两个表plugcode_tag_post(category_post),plugcode_post(post);
     *                     所以最好指定一下表名,以防字段冲突
     *                     limit:数据条数,默认值为10,可以指定从第几条开始,如3,8(表示共调用8条,从第3条开始)
     *                     order:排序方式,如按posts表里的published_time字段倒序排列：post.published_time desc
     *                     where:查询条件,字符串形式,和sql语句一样,请在事先做好安全过滤,最好使用第二个参数$where的数组形式进行过滤,此方法查询时关联多个表,所以最好指定一下表名,以防字段冲突,查询条件(只支持数组),格式和thinkPHP的where方法一样,此方法查询时关联多个表,所以最好指定一下表名,以防字段冲突;
    ...
    public static function tagArticles($param)
    {
        ...
        if (empty($tagId)) {
            ...
            array_push($join, ['__PLUGCODE_TAG_POST__ tag_post', 'post.id = tag_post.post_id']);
