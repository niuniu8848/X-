...
class ProductPostModel extends Model
{
    ...
    {
        return $this->belongsToMany('ProductCategoryModel', 'product_category_post', 'category_id', 'post_id');
    }
    /**
     * 关联标签表
     * @return \think\model\relation\BelongsToMany
    ...
    {
        return $this->belongsToMany('ProductTagModel', 'product_tag_post', 'tag_id', 'post_id');
    }
     ...
     * @return $this
     */
    public function userAddArticle($data, $categories)
    {
        if (!empty($data['more']['thumbnail'])) {
            $data['more']['thumbnail'] = cmf_asset_relative_url($data['more']['thumbnail']);
            $data['thumbnail']         = $data['more']['thumbnail'];
     ...
     * @throws \think\Exception
     */
    public function userEditArticle($data, $categories)
    {
    ...
    {
        $portalTagModel = new ProductTagModel();
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('product_tag_post')->where('post_id', $articleId)->column('tag_id');
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('product_tag_post')->where('post_id', $articleId)->delete();
            }
            ...
            if (!empty($data)) {
                Db::name('product_tag_post')->insertAll($data);
            }
    ...
    public function adminDeletePage($data)
    {
        ...
        if (isset($data['id'])) {
            ...
            if ($res) {
                ...
                $transStatus = false;
                try {
                    Db::name('product_post')->where('id', $id)->update([
                        'delete_time' => time()
                    ]);
            ...
            if ($res) {
                ...
                foreach ($res as $key => $value) {
                    ...
                    $recycleData[$key]['table_name']  = 'product_post';
                ...
                $transStatus = false;
                try {
                    Db::name('product_post')->where('id', 'in', $ids)
                        ->update([
                            'delete_time' => time()
                        ]);
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'product/Page/index', ['id' => $data['id']], 2, 5000);
