...
class Product extends TagLib
{
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::articles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::tagArticles([
    'field'   => {$field},
    'where'   => {$where},
    'limit'   => {$limit},
    'order'   => {$order},
...
<?php
if(!empty({$cid})){
    \$__BREADCRUMB_ITEMS__ = \app\product\service\ApiService::breadcrumb({$cid},{$self});
?>
...
parse;
    ...
    public function tagCategories($tag, $content)
    {
        ...
        $returnVarName = 'product_categories_data';
        $where         = '""';
        if (!empty($tag['where']) && strpos($tag['where'], '$') === 0) {
            $where = $tag['where'];
        }
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::categories([
    'where'   => {$where},
    'order'   => '{$order}',
]);
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'product_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::subCategories({$categoryId});
...
parse;
    ...
    {
        $item          = empty($tag['item']) ? 'vo' : $tag['item'];//循环变量名
        $returnVarName = 'product_all_sub_categories_data';
        ...
        $parse = <<<parse
<?php
\${$returnVarName} = \app\product\service\ApiService::allSubCategories({$categoryId});
 ?>
<volist name="{$returnVarName}" id="{$item}">
{$content}
</volist>
